...
import { useApi, useCheckoutToken } from "@shopify/ui-extensions-react/checkout";
import {useCallback, useState, useEffect} from "react";

export function backendUrl(){
    return "https://multicycle.cpro-server.de"
}
...
export function useAppFetchJson(){
    ...
    return useCallback(
        (path: string, parameters: URLSearchParams, method: "GET" | "POST" = "GET") => {
...
export function useCreateNewConsorsNotifyUUID(){
    ...
    // TODO: Error handling
    return useCallback( () =>     
        appFetchJson(`/api/public/checkouts/${encodeURIComponent(shop.myshopifyDomain)}`, new URLSearchParams({checkoutToken}), "POST")
            .then(jsonReponse => {
...
export function useAppConfig(){
    ...
    useEffect(()=>{
        if(shopDomain!=shop.myshopifyDomain){
            ...
            try{
            appFetchJson(reuestUrl, parameters)
                .then((c: AppConfig) => {
